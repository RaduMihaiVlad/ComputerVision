...
class FacialDetector:
    ...
    def get_positive_descriptors(self):
        ...
        for i in range(num_images):
            ...
            out = hog(img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell), cells_per_block=(2, 2),
                      feature_vector=True)
            positive_descriptors.append(out)

            if self.params.use_flip_images:
                flipped = hog(np.fliplr(img), pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                        cells_per_block=(2, 2), feature_vector=True)
                positive_descriptors.append(flipped)
    ...
    def get_negative_descriptors(self):
        ...
        for i in range(num_images):
            ...
            img = cv.imread(files[i], cv.IMREAD_GRAYSCALE)
            img_height, img_wight = img.shape
            for example in range(num_negative_per_image):
                current_height = np.random.randint(0, img_height - self.params.dim_window)
                current_width = np.random.randint(0, img_wight - self.params.dim_window)

                new_img = img[current_height:(current_height + self.params.dim_window),
                          current_width:(current_width + self.params.dim_window)]

                out = hog(new_img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                          cells_per_block=(2, 2), feature_vector=True)
                negative_descriptors.append(out)

        rest = self.params.number_negative_examples - num_negative_per_image * num_images
        for i in range(rest):
            img = cv.imread(files[i], cv.IMREAD_GRAYSCALE)
            img_height, img_wight = img.shape
    ...
    def get_negative_descriptors_from_array(self, arr):
        ...
        for img in arr:
            img_height, img_wight = img.shape
            if img_height <= self.params.dim_window or img_wight <= self.params.dim_window:
                continue
            for example in range(num_negative_per_image):
                current_height = np.random.randint(0, img_height - self.params.dim_window)
                current_width = np.random.randint(0, img_wight - self.params.dim_window)
    ...
    def intersection_over_union(self, bbox_a, bbox_b):
        ...
        iou = inter_area / float(box_a_area + box_b_area - inter_area + 0.00000001)
    ...
    def run(self, read_path, return_descriptors=False):
        """
        ...
        test_images_path = os.path.join(read_path, '*.jpg')
        ...
        for i in range(num_test_images):
            ...
            file_name = test_files[i].split('\\')[-1]
            ...
            initial_image = copy.deepcopy(img)

            while img.shape[0] >= self.params.dim_window and img.shape[0] >= self.params.dim_window:
                out = hog(img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                          cells_per_block=(2, 2),
                          feature_vector=False)

                new_img = np.reshape(out, (out.shape[0], out.shape[1], 36))
                k = self.params.dim_window // self.params.dim_hog_cell - 1
                l = out.shape[0]
                c = out.shape[1]

                for l1 in range(l - k + 1):
                    for l2 in range(c - k + 1):
                        val = new_img[l1:(l1 + k), l2:(l2 + k)]
                        val = np.reshape(val, (val.shape[0] * val.shape[1] * val.shape[2]))

                        score = (np.dot(val, w) + bias)[0]
                        if score > self.params.threshold:
                            if return_descriptors:
                                left_l1_pos = int((l1 * self.params.dim_hog_cell) // division_value)
                                left_l2_pos = int((l2 * self.params.dim_hog_cell) // division_value)
                                right_l1_pos = int((l1 * self.params.dim_hog_cell + self.params.dim_window) // division_value)
                                right_l2_pos = int((l2 * self.params.dim_hog_cell + self.params.dim_window) // division_value)
                                descriptors_to_return.append(initial_image[left_l1_pos:right_l1_pos, left_l2_pos:right_l2_pos])
                            else:
                                current_scores = np.append(current_scores, score)
                                # file_names = np.append(file_names, i)
                                # current_detection = np.append(current_detection, [[l1, l2, l1 + k, l2 + k]])
                                current_detection.append([(l2 * self.params.dim_hog_cell) // division_value,
                                                          (l1 * self.params.dim_hog_cell) // division_value,
                                                          (l2 * self.params.dim_hog_cell + self.params.dim_window) // division_value,
                                                          (l1 * self.params.dim_hog_cell + self.params.dim_window) // division_value])

                division_value = division_value * self.params.scaling_ratio
                img = cv.resize(img, fx=self.params.scaling_ratio, fy=self.params.scaling_ratio, dsize=None)

            if len(current_detection) > 0:
                new_img_detectinos, new_img_scores = self.non_maximum_suppression(np.array(current_detection),
                                                                                  current_scores, initial_image.shape)
                for x in range(len(new_img_detectinos)):
                    detections = np.append(detections, new_img_detectinos[x])
                    scores = np.append(scores, new_img_scores[x])
        ...
        detections = np.reshape(detections, (len(detections) // 4, 4))
